...
def point_cloud_to_volume_batch(point_clouds,
    ...
    for b in range(point_clouds.shape[0]):
        vol = point_cloud_to_volume(np.squeeze(point_clouds[b, :, :]), vsize,
                                    radius)
        if flatten:
...
def point_cloud_to_volume_v2_batch(point_clouds,
                                   ...
                                   radius=1.0,
                                   num_sample=128):
    """Input is BxNx3 a batch of point cloud Output is BxVxVxVxnum_samplex3
    Added on Feb 19."""
    vol_list = []
    for b in range(point_clouds.shape[0]):
...
def point_cloud_to_volume_v2(points, vsize, radius=1.0, num_sample=128):
    """input is Nx3 points output is vsize*vsize*vsize*num_sample*3 assumes
    points are in range [-radius, radius] samples num_sample points in each
    voxel, if there are less than num_sample points, replicate the points Added
    on Feb 19."""
...
def point_cloud_to_image_batch(point_clouds,
                               ...
                               radius=1.0,
                               num_sample=128):
    """Input is BxNx3 a batch of point cloud Output is BxIxIxnum_samplex3 Added
    on Feb 19."""
    img_list = []
    for b in range(point_clouds.shape[0]):
...
def point_cloud_to_image(points, imgsize, radius=1.0, num_sample=128):
    """input is Nx3 points output is imgsize*imgsize*num_sample*3 assumes
    points are in range [-radius, radius] samples num_sample points in each
    pixel, if there are less than num_sample points, replicate the points Added
    on Feb 19."""
...
def write_ply_color(points,
    ...
    """Color (N,3) points with labels (N) within range 0 ~ num_classes-1 as OBJ
    file."""
...
def write_ply_rgb(points, colors, filename, text=True, num_classes=None):
    """Color (N,3) points with RGB colors (N,3) within range [0,255] as OBJ
    file."""
...
def rotate_pc_along_y(pc, rot_angle):
    """Input ps is NxC points with first 3 channels as XYZ z is facing forward,
    x is left ward, y is downward."""
...
def bbox_corner_dist_measure(crnr1, crnr2):
    ...
    for y in range(4):
        rows = ([(x + y) % 4
                 for x in range(4)] + [4 + (x + y) % 4 for x in range(4)])
...
def point_cloud_to_bbox(points):
    ...
    """
    which_dim = len(
        points.shape) - 2  # first dim if a single cloud and second if batch
...
def write_lines_as_cylinders(pcl, filename, rad=0.005, res=64):
    ...
    for src, tgt in pcl:
        ...
        scene.add_geometry(
            trimesh.creation.cylinder(radius=rad,
                                      height=height,
                                      sections=res,
                                      transform=M))
    mesh_list = trimesh.util.concatenate(scene.dump())
    trimesh.io.export.export_mesh(mesh_list,
                                  f'{filename}.ply',
                                  file_type='ply')

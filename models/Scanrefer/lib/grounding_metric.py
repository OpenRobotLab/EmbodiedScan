...
def ground_eval_subset(gt_anno_list, det_anno_list, logger=None, prefix=''):
    ...
    ious = [
    ]  # each pred box has a ious, shape (num_gt) in the corresponding sample
    ...
    for sample_idx in tqdm(range(num_samples)):
        ...
        iou_mat = euler_iou3d_split(pred_center, pred_size, pred_rot,
    ...
    subset_result = {
        ...
        'ious':
        ious,  # list, num_preds, each element is a num_gt (changing) np.ndarray
...
def ground_eval_overall(gt_anno_list, det_anno_list, logger=None):
    ...
    reference_options = list(
        set(abbr(gt_anno['sub_class']) for gt_anno in gt_anno_list))
    reference_options.sort()
    assert len(det_anno_list) == len(gt_anno_list)
    full_result = {}
    for ref in reference_options:
        indices = [
            i for i, gt_anno in enumerate(gt_anno_list)
            if abbr(gt_anno['sub_class']) == ref
        ]
        sub_gt_annos = [gt_anno_list[i] for i in indices]
        sub_det_annos = [det_anno_list[i] for i in indices]
        subset_results = ground_eval_subset(sub_gt_annos,
                                            sub_det_annos,
                                            logger=logger,
                                            prefix=ref)
    ...
    prev_samples = 0
    for ref in reference_options:
        full_result['overall']['confidences'].extend(
            full_result[ref]['confidences'])
        full_result['overall']['sample_indices'].extend(
            [x + prev_samples for x in full_result[ref]['sample_indices']])
        full_result['overall']['ious'].extend(full_result[ref]['ious'])
        full_result['overall']['num_gts_per_sample'].extend(
            full_result[ref]['num_gts_per_sample'])
        prev_samples += len(full_result[ref]['num_gts_per_sample'])
    return full_result


def compute_metric_subset(subset_results,
                          iou_thr=[0.25, 0.5],
                          logger=None,
                          prefix=''):
    assert isinstance(subset_results,
                      dict), f'expected a dict, but got {type(subset_results)}'
    ...
    for t in iou_thr:
        ...
        f1s = 2 * recall * precision / np.maximum(recall + precision,
...
def ground_eval(gt_anno_list, det_anno_list, logger=None):
    ...
    iou_thr = [0.25, 0.5]
    reference_options = [
        abbr(gt_anno.get('sub_class', 'other')) for gt_anno in gt_anno_list
    ]
    ...
    assert len(det_anno_list) == len(gt_anno_list)
    metric_results = {}
    mid_result = ground_eval_overall(gt_anno_list,
                                     det_anno_list,
                                     logger=logger)
    for ref in reference_options:
        metric_results.update(
            compute_metric_subset(mid_result[ref],
                                  iou_thr=iou_thr,
                                  logger=logger,
                                  prefix=ref))
    metric_results.update(
        compute_metric_subset(mid_result['overall'],
                              iou_thr=iou_thr,
                              logger=logger,
                              prefix='overall'))

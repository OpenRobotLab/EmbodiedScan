...
from lib.euler_utils import (axis_aligned_bbox_overlaps_3d, bbox_to_corners,
                             chamfer_distance, euler_iou3d)
from lib.loss import SigmoidRankingLoss, SoftmaxRankingLoss
from utils.box_util import (box3d_iou, box3d_iou_batch, get_3d_box,
                            get_3d_box_batch)
...
def compute_vote_loss(data_dict):
    ...
    seed_gt_votes_mask = torch.gather(data_dict['vote_label_mask'], 1,
                                      seed_inds)
...
def compute_box_and_sem_cls_loss(data_dict, config):
    ...
    target_rot_mat = torch.gather(
        data_dict['target_rot_mat'], 1,
        object_assignment.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 3,
                                                             3))  # B, K, 3x3
    target_bbox = torch.gather(data_dict['target_bbox'], 1,
                               object_assignment.unsqueeze(-1).repeat(
                                   1, 1, 6))  # B, K, 6
    pred_rot_mat = data_dict['rot_mat']
    pred_corners = bbox_to_corners(pred_center, pred_size, pred_rot_mat)
    target_corners = bbox_to_corners(target_bbox[:, :, :3],
                                     target_bbox[:, :, 3:6], target_rot_mat)
    bbox_cd_loss, _, _, _ = chamfer_distance(pred_corners, target_corners, 1.0,
                                             0.0, 'l1', 'mean')
...
def compute_reference_loss(data_dict, config):
    ...
    gt_corners = bbox_to_corners(gt_center.to(torch.float64),
                                 gt_size.to(torch.float64),
                                 gt_rot_mat.to(torch.float64)).to(
                                     torch.float32)
...
def get_loss(data_dict,
    ...
    center_loss, bbox_cd_loss, size_cls_loss, size_reg_loss, sem_cls_loss = compute_box_and_sem_cls_loss(
        data_dict, config)
    ...
    if reference:  # TODO yesname
        ...
        # ref_loss, _, cluster_labels = compute_reference_loss(data_dict, config)
        # data_dict["cluster_labels"] = cluster_labels
        data_dict['cluster_labels'] = objectness_label.new_zeros(
            objectness_label.shape).cuda()
        data_dict['cluster_ref'] = objectness_label.new_zeros(
            objectness_label.shape).float().cuda()

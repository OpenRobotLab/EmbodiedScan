...
def check_aspect(crop_range, aspect_min):
    ...
    return ((xy_aspect >= aspect_min) or (xz_aspect >= aspect_min)
...
class RandomCuboid(object):
    ...
    [https://arxiv.org/abs/2101.02691] We slightly modify this operation to
    account for object detection.

    This augmentation randomly crops a cuboid from the input and ensures
    that the cropped cuboid contains at least one bounding box
    """
    ...
    def __call__(self, point_cloud, target_boxes, per_point_labels=None):
        ...
        for _ in range(100):
            ...
            sample_center = point_cloud[np.random.choice(len(point_cloud)),
                                        0:3]
            ...
            upper_idx = (np.sum(
                (point_cloud[:, 0:3] <= max_xyz).astype(np.int32), 1) == 3)
            lower_idx = (np.sum(
                (point_cloud[:, 0:3] >= min_xyz).astype(np.int32), 1) == 3)
            ...
            if self.box_filter_policy == 'center':
                ...
                if (
                    ...
                    new_pc_min_max = np.min(new_point_cloud[:, 0:3],
                                            axis=0), np.max(
                                                new_point_cloud[:, 0:3],
                                                axis=0)
                ...
                if per_point_labels is not None:
                    new_per_point_labels = [
                        x[new_pointidx] for x in per_point_labels
                    ]
                else:

...
Collected and written by Charles R. Qi Last modified: Apr 2021 by Ishan Misra
"""
import numpy as np
import torch
...
def box3d_iou_batch_tensor(corners1, corners2):
    ...
    x_min_1, x_max_1, y_min_1, y_max_1, z_min_1, z_max_1 = get_box3d_min_max_batch_tensor(
        corners1)
    ...
    inter_vol = torch.max((xB - xA), zeros) * torch.max(
        (yB - yA), zeros) * torch.max((zB - zA), zeros)

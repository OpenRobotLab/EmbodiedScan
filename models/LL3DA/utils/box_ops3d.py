...
def helper_computeIntersection(cp1: torch.Tensor, cp2: torch.Tensor,
...
def polygon_clip_unnest(subjectPolygon: torch.Tensor,
        ...
        for subjectVertex in inputList:
            ...
            if helper_inside(cp1, cp2, e):
                if not helper_inside(cp1, cp2, s):
                    outputList.append(
                        helper_computeIntersection(cp1, cp2, s, e))
                outputList.append(e)
...
def generalized_box3d_iou_tensor_non_diff(corners1: torch.Tensor,
    ...
    if batch_intersect is None:
        ...
        ymax = torch.min(corners1[:, :, 0, 1][:, :, None],
                         corners2[:, :, 0, 1][:, None, :])
        ymin = torch.max(corners1[:, :, 4, 1][:, :, None],
                         corners2[:, :, 4, 1][:, None, :])
        ...
        lt = torch.max(rect1[:, :, 1][:, :, None, :], rect2[:, :,
                                                            1][:, None, :, :])
        rb = torch.min(rect1[:, :, 3][:, :, None, :], rect2[:, :,
                                                            3][:, None, :, :])
        ...
        if rotated_boxes:
            ...
            batch_intersect(rect1, rect2, non_rot_inter_areas_np, nums_k2_np,
                            inter_areas, approximate)
...
def generalized_box3d_iou_tensor(corners1: torch.Tensor,
                                 corners2: torch.Tensor,
                                 nums_k2: torch.Tensor,
                                 rotated_boxes: bool = True,
                                 return_inter_vols_only: bool = False,
...
generalized_box3d_iou_tensor_jit = torch.jit.script(
    generalized_box3d_iou_tensor)


def enclosing_box3d_convex_hull(corners1,
                                corners2,
                                nums_k2,
                                mask,
                                enclosing_vols=None):
    ...
    for b in range(B):
        for k1 in range(K1):
            for k2 in range(K2):
                ...
                hull = ConvexHull(np.vstack([corners1[b, k1], corners2[b,
                                                                       k2]]))
...
def generalized_box3d_iou_convex_hull_nondiff_tensor(
    ...
    inter_vols = generalized_box3d_iou_tensor_jit(corners1,
                                                  corners2,
                                                  nums_k2,
                                                  rotated_boxes,
        ...
        for i in range(K1):
            for j in range(K2):
                if nums_k2 is not None and j >= nums_k2[b]:
                    break
                iou, sum_of_vols = box3d_iou(corners1_np[b, i], corners2_np[b,
                                                                            j])

                hull = ConvexHull(
                    np.vstack([corners1_np[b, i], corners2_np[b, j]]))
                C = hull.volume
...
def get_3d_box_batch_tensor(box_size, heading_angle, center):
    ...
    corners_3d = torch.zeros(tuple(list(input_shape) + [8, 3]),

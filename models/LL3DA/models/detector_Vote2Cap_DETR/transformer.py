...
import torch
from models.detector_Vote2Cap_DETR.helpers import (ACTIVATION_DICT, NORM_DICT,
                                                   WEIGHT_INIT_DICT,
                                                   get_clones)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_fn_name is not None:
            self.norm = NORM_DICT[norm_fn_name](
                self.layers[0].linear2.out_features)
    ...
    def forward(
        ...
        if transpose_swap:
            ...
            memory = memory.flatten(2).permute(
                2, 0, 1)  # memory: bs, c, t -> t, b, c
            if pos is not None:
                pos = pos.flatten(2).permute(2, 0, 1)
        output = tgt
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.masking_radius[idx] > 0:
                mask, xyz_dist = self.compute_mask(xyz,
                                                   self.masking_radius[idx],
                                                   xyz_dist)
                # mask must be tiled to num_heads of the transformer
                bsz, n, n = mask.shape
                nhead = layer.nhead

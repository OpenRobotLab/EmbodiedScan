        ...
        if is_primary() and curr_iter % args.log_every == 0:
            ...
            if curr_iter % 200 == 0:
                with open(
                        os.path.join(
                            args.checkpoint_dir,
                            f'embodiedscan_L_pred_gt_val_{curr_iter}.json'),
                        'w') as f:
                    pred_gt_val = {}
                    for scene_object_id, scene_object_id_key in enumerate(
                            candidates):
    ...
    if is_primary():
        ...
        with open(
                os.path.join(args.checkpoint_dir,
                             'embodiedscan_L_corpus_val.json'), 'w') as f:
            json.dump(corpus, f, indent=4)

        with open(
                os.path.join(args.checkpoint_dir,
                             'embodiedscan_L_pred_val.json'), 'w') as f:
            json.dump(candidates, f, indent=4)

        with open(
                os.path.join(args.checkpoint_dir,
                             'embodiedscan_L_pred_gt_val.json'), 'w') as f:
            pred_gt_val = {}

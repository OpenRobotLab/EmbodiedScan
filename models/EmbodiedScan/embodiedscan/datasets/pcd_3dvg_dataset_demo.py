...
def build_demo_info(demo_scene_id='office'):
    ...
    for i in range(1, n_frames):
        ...
        image_ann = dict(img_path=os.path.join('demo', demo_scene_id, 'rgb',
                                               timestamp + '.jpg'),
                         depth_path=os.path.join('demo', demo_scene_id,
                                                 'depth', timestamp + '.png'),
...
class PointCloud3DGroundingDatasetDemo(BaseDataset):
    ...
    def load_language_data(self):
        ...
        language_infos = []
        print('number of scans loaded from info data:',
              len(list(self.scans.keys())))
        ...
        for anno in mmengine.track_iter_progress(language_annotations):
            ...
            labels = ann_info.get('gt_labels_3d',
                                  np.zeros(0))  # all box labels in the scan
            bboxes = ann_info.get('gt_bboxes_3d', np.zeros(
                (0, 9)))  # BaseInstanceBboxes

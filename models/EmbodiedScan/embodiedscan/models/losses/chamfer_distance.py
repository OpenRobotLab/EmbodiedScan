...
def bbox_to_corners(bbox: Tensor) -> Tensor:
    ...
    eight_corners = torch.tensor(eight_corners,
                                 device=device).float()  # shape (8, 3)
    eight_corners = (1 - 2 * torch.tensor(
        eight_corners, device=device).float()) * half_sizes  # shape (8, 3)

...
- #### Data Access
    ...
    <details>
    <summary>(2) Language Modality</summary>



    <details>
    <summary>Category and ID info</summary>
    ...
    <details>
    <summary> For Visual Grounding task </summary>
    ...
    <details>
    <summary> For Question Answering task </summary>
...
- #### Question Answering Evaluator
    ...
    my_evaluator = QuestionAnsweringEvaluator(model_config={}, show_results=True)
...
### MMScan Workshop Submission

   The result you submit must be a dictionary, where the key is the `sample ID` from the test split. For each sample, include your **predicted 100 bounding boxes** (which do not need to be sorted by confidence scores) along with their corresponding **confidence scores** as the values of keys {`pred_bboxes`, `scores`}. To reduce the time overhead during file transfer, all floating-point numbers in the data should be rounded to **two decimal places**.
   ...
   *Note:* For submission results that exceed the bounding box limit or the floating-point precision limit, we will **truncate them** during evaluation to ensure fairness.
